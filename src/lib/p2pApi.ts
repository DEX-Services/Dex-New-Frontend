...
export const P2P_ASSETS = ["USDB", "USDC"] as const;
...
export type P2PListing = { id:string; sellerId:string; sellerAddress:string; asset:P2PAsset; amountRaw:string; remainingRaw:string; price:string; fiatCurrency:string; paymentMethod:P2PPaymentMethod; status:"ACTIVE"|"FILLED"|"CANCELLED"; createdAt:string; updatedAt:string };
...
export const createP2PListing=(asset:P2PAsset,amountRaw:string,paymentMethod:P2PPaymentMethod)=>request<{listing:P2PListing}>("/p2p/listings",json({asset,amountRaw,paymentMethod}));
export const buyP2PListing=(listingId:string,amountRaw:string)=>request<{order:P2POrder}>("/p2p/buy",json({listingId,amountRaw,idempotencyKey:idempotencyKey()}));
...
export const parseUSDC=(value:string)=>parseP2PAmount(value,"USDC");
export const formatUSDC=formatP2PAmount;
