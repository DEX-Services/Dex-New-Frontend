...
import { createP2PListing,formatINR,getP2PPaymentAccounts,parseUSDBAmount,P2P_PAYMENT_METHODS,type P2PAdSide,type P2PListing,type P2PPaymentMethod } from "@/lib/p2pApi";
...
export function PostAdsDialog({open,onOpenChange,side,username,price,onUsernameEstablished,onCreated}:Props){
	...
	function normalizeAmount(){const value=Number(amount);const normalized=Number.isFinite(value)&&value>=0?value:0;const nextTotal=normalized*Number(price||0);setAmount(normalized.toFixed(2));if(Number(maxOrderFiat)<=0||Number(maxOrderFiat)>nextTotal)setMaxOrderFiat(nextTotal.toFixed(2))}
	...
	async function submit(){
		try{
			...
			const {listing}=await createP2PListing(side,parseUSDBAmount(amount),minOrderFiat,maxOrderFiat,methods,username?undefined:name.trim());
	...
	return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto"><DialogHeader><DialogTitle>Post {side==="BUY"?"Buy":"Sell"} USDB ad</DialogTitle><DialogDescription>Set the total amount, per-order limits, and accepted payment methods.</DialogDescription></DialogHeader><div className="space-y-5">
		...
		<div className="space-y-3"><label className="text-sm font-medium">Payment methods</label><div className="grid grid-cols-1 gap-2 rounded-lg border p-4 sm:grid-cols-2">{P2P_PAYMENT_METHODS.map(method=>{const disabled=side==="SELL"&&!configuredMethods.includes(method);return <label key={method} className={`flex items-center gap-2 text-sm ${disabled?"cursor-not-allowed text-muted-foreground":"cursor-pointer"}`}><Checkbox checked={methods.includes(method)} disabled={disabled} onCheckedChange={()=>toggleMethod(method)}/><span>{method}{disabled?" (not configured)":""}</span></label>})}</div><p className="text-xs text-muted-foreground">{side==="SELL"?"Configure receiving accounts in P2P Wallet, then select the methods for this ad.":"Select the methods you can use to pay a counterparty seller."}</p></div>
		{side==="SELL"&&<p className="text-xs text-muted-foreground">Posting reserves {(Number(amount||0)*1.01).toFixed(2)} USDB from your P2P wallet, including the 1% seller fee. Transfer funds separately before posting.</p>}
