...
import { createP2PListing,formatINR,getP2PPaymentAccounts,parseBIUSDBAmount,P2P_PAYMENT_METHODS,sellerBIUSDBDebitWithFee,type P2PAdSide,type P2PListing,type P2PPaymentMethod } from "@/lib/p2pApi";
...
export function PostAdsDialog({open,onOpenChange,side,username,price,onUsernameEstablished,onCreated}:Props){
	...
	async function submit(){
		try{
			...
			const {listing}=await createP2PListing(side,parseBIUSDBAmount(amount),minOrderFiat,maxOrderFiat,methods,username?undefined:name.trim());
	...
	return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto"><DialogHeader><DialogTitle>Post {side==="BUY"?"Buy":"Sell"} BIUSDB ad</DialogTitle><DialogDescription>Set the total amount, per-order limits, and accepted payment methods.</DialogDescription></DialogHeader><div className="space-y-5">
		...
		<div className="space-y-2"><label className="text-sm font-medium">BIUSDB amount to {side==="BUY"?"buy":"sell"}</label><div className="flex gap-2"><Input aria-label="BIUSDB amount" inputMode="decimal" value={amount} onChange={event=>updateAmount(event.target.value)} onBlur={normalizeAmount}/><span className="flex items-center rounded-md bg-muted/30 px-3">BIUSDB</span></div>
		...
		{/* {side==="SELL"&&<p className="text-xs text-muted-foreground">Posting reserves {sellerBIUSDBDebitWithFee(amount)} BIUSDB from your P2P wallet, including the 1% seller fee. Transfer funds separately before posting.</p>} */}
