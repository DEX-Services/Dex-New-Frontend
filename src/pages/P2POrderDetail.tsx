...
export default function P2POrderDetail() {
  ...
  return <AppShell><main className="mx-auto min-h-screen max-w-7xl space-y-6 p-6">
    ...
    {!order ? <Card className="p-8 text-center text-muted-foreground">Loading order…</Card> : <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
      <div className="space-y-6">
        <Card className="border-border/50 bg-card/30 p-6"><div className="mb-5 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary"/><h2 className="font-semibold">Payment and order details</h2></div>
          {buyer && <div className="mb-5 rounded-lg border bg-muted/20 p-4"><Detail label="Transfer via" value={order.paymentMethod}/><Detail label="Account name" value={order.paymentAccountName}/><Detail label="Payment identifier" value={order.paymentAccountIdentifier}/>{order.paymentInstructions && <Detail label="Instructions" value={order.paymentInstructions}/>}</div>}
          <div className="grid gap-x-8 sm:grid-cols-2"><Detail label="Fiat amount" value={formatINR(order.grossAmount)}/><Detail label="Price" value={`${formatINR(order.price)} / USDB`}/><Detail label="Trade quantity" value={`${formatUSDBAmount(order.amountRaw)} USDB`}/><Detail label="Your 1% fee" value={`${formatUSDBAmount(buyer ? order.buyerFeeRaw : order.sellerFeeRaw)} USDB`}/><Detail label={buyer ? "You receive" : "USDB escrowed"} value={`${formatUSDBAmount(buyer ? order.buyerCreditRaw : order.sellerDebitRaw)} USDB`}/><Detail label="Payment deadline" value={new Date(order.expiresAt).toLocaleString()}/></div>
        </Card>
    ...
    <Dialog open={releaseOpen} onOpenChange={setReleaseOpen}><DialogContent><DialogHeader><DialogTitle>Release USDB?</DialogTitle><DialogDescription>Check your bank or payment account directly. A screenshot or chat message alone is not proof that money was received.</DialogDescription></DialogHeader><Button disabled={acting} onClick={() => void perform(() => releaseP2POrder(orderId), () => setReleaseOpen(false))}>I received the payment — release USDB</Button></DialogContent></Dialog>
