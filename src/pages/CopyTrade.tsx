...
function CopySetupDialog({ trader, relationship, open, onOpenChange, onSave }: { trader: Trader | null; relationship?: CopyRelationship; open: boolean; onOpenChange: (open: boolean) => void; onSave: (relationship: CopyRelationship) => void }) {
  ...
  const save = () => {
    ...
    if (!Number.isFinite(allocationValue) || allocationValue < 100) { toast.error("Enter an allocation of at least 100 USDB"); return; }
    if (!Number.isFinite(maxTradeValue) || maxTradeValue <= 0 || maxTradeValue > allocationValue) { toast.error("Max per trade must be between 1 USDB and your allocation"); return; }
      ...
      <DialogContent className="copy-trading-theme glass-strong max-h-[92vh] max-w-xl overflow-y-auto border-primary/20 p-0">
        ...
        <div className="space-y-5 p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div><div className="mb-2 flex items-center justify-between"><label htmlFor="max-per-trade" className="text-sm font-medium">Max per trade</label><span className="text-xs text-muted-foreground">USDB</span></div><Input id="max-per-trade" type="number" min="1" value={maxPerTrade} onChange={(event) => setMaxPerTrade(event.target.value)} className="h-11 border-white/10 bg-white/[0.035] font-mono" /></div>
          </div>
