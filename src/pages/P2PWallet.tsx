...
export default function P2PWallet() {
  ...
  const regularAvailable = useMemo(
    () => balances.find((balance) => balance.asset === "BIUSDB")?.available ?? 0,
    [balances],
  );
  ...
  async function transfer() {
    ...
    try {
      ...
      setTransferError("");
      setTransferSuccess("");
      const response = await fundP2PWallet("BIUSDB", parseBIUSDBAmount(amount));
      setP2PBalance(response.balance);
      setTransferSuccess(`${displayedAmount} BIUSDB transferred to your P2P wallet.`);
      setAmount("0");
      try {
      ...
      setTransferError(cause instanceof Error ? cause.message : "Could not transfer BIUSDB");
    } finally {
      setLoading(false);
    }
  }
    ...
    <AppShell>
      <main className="mx-auto min-h-screen max-w-5xl space-y-6 p-6">
        <div>
          ...
          <p className="text-muted-foreground">Move BIUSDB from your Balance Wallet before using it for P2P selling.</p>
        </div>
        ...
        {!userId ? (
          ...
          <>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Kpi label="Balance Wallet" value={`${Number(regularAvailable.toFixed(6))} BIUSDB`} />
              <Kpi label="P2P Wallet Balance" value={`${formatBIUSDBAmount(p2pBalance.totalRaw)} BIUSDB`} />
              <Kpi label="Available for Sale" value={`${formatBIUSDBSellCapacity(p2pBalance.availableRaw)} BIUSDB`} />
              <Kpi label="Reserved in Ads" value={`${formatBIUSDBAmount(p2pBalance.reservedRaw)} BIUSDB`} />
            </div>
            ...
            <Card className="border-border/50 bg-card/30 p-6">
              ...
              <div className="max-w-xl space-y-4">
                <div>
                  <label className="mb-2 block text-sm font-medium">BIUSDB amount</label>
                  <div className="relative">
                    <Input
                      ...
                      className="pr-20"
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-semibold">BIUSDB</span>
                  </div>
                  <p className="mt-2 text-xs text-muted-foreground">Available in Balance Wallet: {Number(regularAvailable.toFixed(6))} BIUSDB</p>
                </div>
