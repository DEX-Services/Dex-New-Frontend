...
import { effectiveP2PMaxOrderFiat,formatINR,formatUSDBAmount,getP2PListings,getP2PPrice,grossUSDBAmountForNet,netUSDBAmountAfterBuyerFee,parseUSDBAmount,P2P_PAYMENT_METHODS,takeP2PListing,usdbAmountFromFiat,type P2PListing,type P2POrder,type P2PPaymentMethod } from "@/lib/p2pApi";

const message=(error:unknown)=>error instanceof Error?error.message:"Something went wrong";
const validUSDBInput=(value:string)=>/^\d*(?:\.\d{0,2})?$/.test(value);
...
export default function P2P(){
	...
	const [quantity,setQuantityState]=useState("0.00");
	...
	function open(ad:P2PListing){const minimum=Math.ceil((Number(ad.minOrderFiat)/Number(ad.price))*100)/100;setSelected(ad);setQuantity(Math.min(minimum,Number(formatUSDBAmount(ad.remainingRaw))).toFixed(2));setSelectedPayment(ad.paymentMethods[0]);setOrder(null);setError("")}
	...
	return <AppShell><div className="min-h-screen bg-gradient-to-b from-background via-background/95 to-background p-6"><div className="mx-auto max-w-7xl">
		...
		<div className="grid grid-cols-1 gap-6 lg:grid-cols-3"><div className="space-y-6 lg:col-span-2"><div className="flex gap-3"><Button onClick={()=>setAction("BUY")} className={`h-11 min-w-36 px-10 font-semibold ${action==="BUY"?"bg-buy text-buy-foreground":"bg-muted text-muted-foreground"}`}>Buy USDB</Button><Button onClick={()=>setAction("SELL")} variant={action==="SELL"?"default":"outline"} className={`h-11 min-w-36 px-10 font-semibold ${action==="SELL"?"bg-red-500 text-white":"border border-border"}`}>Sell USDB</Button></div>
			...
			<Card className="border-border/50 bg-card/30 p-6"><h3 className="mb-4 flex items-center gap-2 font-semibold"><TrendingUp className="h-4 w-4 text-primary"/>Quick Trade</h3><Input value={amount} onChange={event=>setAmount(event.target.value)} placeholder="0.00 USDB"/><div className="my-4 rounded-lg bg-muted/20 p-3 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Available across ads</span><span>{total.toFixed(2)} USDB</span></div></div><Button disabled={!match} onClick={()=>match&&open(match)} className={`w-full ${action==="BUY"?"bg-buy text-buy-foreground":"bg-red-500 text-white"}`}>{action==="BUY"?"Proceed to Buy":"Proceed to Sell"}<ArrowRight className="ml-2 h-4 w-4"/></Button></Card>
...
function TradeDialog({ad,quantity,setQuantity,payment,setPayment,order,error,acting,userId,onTake,onClose}:{ad:P2PListing|null;quantity:string;setQuantity:(v:string)=>void;payment:P2PPaymentMethod;setPayment:(v:P2PPaymentMethod)=>void;order:P2POrder|null;error:string;acting:boolean;userId?:string;onTake:()=>void;onClose:()=>void}){
	const [linkedInputs,setLinkedInputs]=useState({adId:"",fiat:"0.00",receive:"0.00"});
	...
	const fee=(amount*.01).toFixed(2);
	const viewerAction=ad?.side==="SELL"?"Buy":"Sell";
	const valid=!!ad&&userId!==ad.creatorId&&amount>0&&amount<=remaining&&gross>=Number(ad.minOrderFiat)&&gross<=effectiveMaximum&&(remainingAfter<=0||remainingAfter*Number(ad.price)>=Number(ad.minOrderFiat));
	...
	function normalizeNetUSDB(){updateNetUSDB((Number(receiveInput)||0).toFixed(2))}
	function normalizeGrossUSDB(){setQuantity((Number(quantity)||0).toFixed(2))}
	function close(){setLinkedInputs({adId:"",fiat:"0.00",receive:"0.00"});onClose()}
	return <TradeDialogView ad={ad} quantity={quantity} setQuantity={setQuantity} fiatInput={fiatInput} receiveInput={receiveInput} payment={payment} setPayment={setPayment} order={order} error={error} acting={acting} userId={userId} onTake={onTake} onClose={close} amount={amount} gross={gross} effectiveMaximum={effectiveMaximum} fee={fee} viewerAction={viewerAction} valid={valid} updateFiatInvestment={updateFiatInvestment} updateNetUSDB={updateNetUSDB} normalizeFiatInvestment={normalizeFiatInvestment} normalizeNetUSDB={normalizeNetUSDB} normalizeGrossUSDB={normalizeGrossUSDB}/>;
}
...
function TradeDialogView({ad,quantity,setQuantity,fiatInput,receiveInput,payment,setPayment,order,error,acting,userId,onTake,onClose,amount,gross,effectiveMaximum,fee,viewerAction,valid,updateFiatInvestment,updateNetUSDB,normalizeFiatInvestment,normalizeNetUSDB,normalizeGrossUSDB}:TradeDialogViewProps){
		...
		<DialogContent className="max-w-lg">
			...
			{ad&&(order?<div className="space-y-4">
				...
				{viewerAction==="Buy"?<div className="space-y-4">
					...
					<label className="mb-2 block text-xs font-semibold uppercase text-muted-foreground">USDB quantity</label>
					<div className="flex gap-2"><Input inputMode="decimal" value={quantity} onChange={event=>setQuantity(event.target.value)} onBlur={normalizeGrossUSDB}/><span className="flex min-w-20 items-center justify-center rounded-md bg-muted/30 px-3">USDB</span></div>
					<p className="mt-1 text-xs text-muted-foreground">Order value must be within {formatINR(ad.minOrderFiat)} – {formatINR(effectiveMaximum)}.</p>
				</div>}
				<div>
				...
				<div className="rounded-xl border p-4 text-sm">
					...
					<Row label={viewerAction==="Buy"?"You receive":"Total USDB escrowed"} value={`${viewerAction==="Buy"?netUSDBAmountAfterBuyerFee(amount):(amount*1.01).toFixed(2)} USDB`}/>
				</div>
				{error&&<p className="text-sm text-destructive">{error}</p>}
				<Button className={`w-full ${viewerAction==="Buy"?"bg-buy text-buy-foreground":"bg-red-500 text-white"}`} disabled={!valid||acting} onClick={onTake}>{acting?"Creating order…":viewerAction==="Buy"?`Buy ${netUSDBAmountAfterBuyerFee(amount)} USDB`:`Sell ${amount.toFixed(2)} USDB`}</Button>
			</div>)}
		</DialogContent>
	</Dialog>;
}
