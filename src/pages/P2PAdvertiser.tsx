...
import { cancelP2PListing,effectiveP2PMaxOrderFiat,formatINR,formatUSDBAmount,formatUSDBSellCapacity,getMyP2PListings,getP2PPrice,getP2PProfile,getP2PWallet,type P2PAdSide,type P2PListing,type P2PWalletBalance } from "@/lib/p2pApi";
...
export default function P2PAdvertiser(){
	...
	return <AppShell><main className="mx-auto min-h-screen max-w-6xl space-y-6 p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><Link to="/p2p" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary"><ArrowLeft className="h-4 w-4"/>Back to Marketplace</Link><h1 className="mt-4 text-3xl font-bold">My Ads</h1><p className="text-muted-foreground">Manage your Buy USDB and Sell USDB ads.</p></div><div className="flex gap-2"><Button asChild variant="outline"><Link to="/p2p/wallet"><Wallet className="mr-2 h-4 w-4"/>P2P Wallet</Link></Button><DropdownMenu><DropdownMenuTrigger asChild><Button>Post Ad<ChevronDown className="ml-2 h-4 w-4"/></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={()=>post("BUY")}>Buy USDB</DropdownMenuItem><DropdownMenuItem onClick={()=>post("SELL")}>Sell USDB</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></div>
		{!userId?<Card className="p-8 text-center text-muted-foreground">Connect and authenticate a wallet to view your ads.</Card>:<><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="P2P Wallet Balance" value={`${formatUSDBAmount(balance.totalRaw)} USDB`}/><Kpi label="Current Price" value={`${formatINR(price)} / USDB`}/><Kpi label="Available for Sale" value={`${formatUSDBSellCapacity(balance.availableRaw)} USDB`}/><Kpi label="Active Ads" value={String(active.length)}/></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><Kpi label="Reserved in Sell Ads" value={`${formatUSDBAmount(balance.reservedRaw)} USDB`}/><Kpi label="P2P Username" value={username||"Set when posting your first ad"}/></div>{error&&<Card className="p-4 text-destructive">{error}</Card>}<Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-sm"><thead className="border-b bg-muted/30 text-left text-xs uppercase text-muted-foreground"><tr><th className="p-4">Created</th><th className="p-4">Ad</th><th className="p-4">Price</th><th className="p-4">Progress</th><th className="p-4">Order limits</th><th className="p-4">Payment methods</th><th className="p-4">Status</th><th className="p-4 text-right">Action</th></tr></thead><tbody>{loading?<tr><td colSpan={8} className="p-8 text-center">Loading…</td></tr>:listings.length===0?<tr><td colSpan={8} className="p-8 text-center text-muted-foreground">You have not posted any ads.</td></tr>:listings.map(ad=><tr className="border-b last:border-0" key={ad.id}><td className="p-4">{new Date(ad.createdAt).toLocaleString()}</td><td className="p-4 font-semibold">{ad.side} USDB</td><td className="p-4">{formatINR(ad.price)}</td><td className="p-4"><div className="font-medium">{formatUSDBAmount(ad.completedAmountRaw)} / {formatUSDBAmount(ad.amountRaw)} USDB</div><div className="text-xs text-muted-foreground">{ad.completedOrders} completed · {formatUSDBAmount(ad.remainingRaw)} remaining</div></td><td className="p-4">{formatINR(ad.minOrderFiat)} – {formatINR(effectiveP2PMaxOrderFiat(ad))}</td><td className="p-4"><div className="flex flex-wrap gap-1">{ad.paymentMethods.map(method=><Badge key={method} variant="secondary">{method}</Badge>)}</div></td><td className="p-4">{ad.status}</td><td className="p-4 text-right"><Button size="sm" variant="outline" disabled={ad.status!=="ACTIVE"} onClick={()=>void cancel(ad.id)}>Cancel</Button></td></tr>)}</tbody></table></div></Card></>}
