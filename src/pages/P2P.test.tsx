...
describe("P2P page", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      ...
      const body = url.includes("/p2p/price")
        ? { price: { asset: "BIUSDB", fiatCurrency: "INR", price: "100.00000000", priceDate: "2026-07-16", createdAt: new Date().toISOString() } }
        : { listings };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "Content-Type": "application/json" } }));
    }));
  });

  it("renders the BIUSDB marketplace without a Post Ad action", async () => {
    render(<MemoryRouter><P2P /></MemoryRouter>);
    expect(await screen.findByRole("heading", { name: "Buy BIUSDB" })).toBeInTheDocument();
    expect(await screen.findByText("No matching BIUSDB ads found.")).toBeInTheDocument();
  ...
  it("links the buyer's INR payment and net BIUSDB receipt fields", async () => {
    listings = [{
      id: "listing-1", creatorId: "seller-1", username: "SellerOne", side: "SELL", asset: "BIUSDB",
    ...
    fireEvent.click(screen.getAllByRole("button", { name: "BUY BIUSDB" }).at(-1)!);

    const payInput=screen.getByRole("textbox", { name: "You pay in INR" }) as HTMLInputElement;
    const receiveInput=screen.getByRole("textbox", { name: "You receive in BIUSDB" }) as HTMLInputElement;
  ...
  it("accepts an INR amount in Quick Trade and opens the matching buy offer", async () => {
    listings = [{
      id: "listing-quick", creatorId: "seller-1", username: "QuickSeller", side: "SELL", asset: "BIUSDB",
      amountRaw: "5000000", remainingRaw: "5000000", price: "100.00", fiatCurrency: "INR",
      minOrderFiat: "100.00", maxOrderFiat: "350.00", paymentMethods: ["UPI"], status: "ACTIVE",
      completedOrders: 0, completedAmountRaw: "0", completedOrders30d: 0, completionRate30d: "0.00",
      ratedOrders30d: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    ...
    expect(screen.getByRole("textbox", { name: "Quick trade BIUSDB amount" })).toHaveValue("2.5");
    ...
    expect(screen.getByRole("textbox", { name: "You receive in BIUSDB" })).toHaveValue("2.475");
  });

  it("links the seller's INR receipt and BIUSDB amount and explains an unusable remainder", async () => {
    listings = [{
      id: "listing-2", creatorId: "buyer-1", username: "DexUser215", side: "BUY", asset: "BIUSDB",
    ...
    render(<MemoryRouter><P2P /></MemoryRouter>);
    fireEvent.click(screen.getByRole("button", { name: "Sell BIUSDB" }));
    expect(await screen.findByText("DexUser215")).toBeInTheDocument();
    fireEvent.change(screen.getByRole("textbox", { name: "Quick trade BIUSDB amount" }), { target: { value: "6.05" } });
    ...
    const sellInput=screen.getByRole("textbox", { name: "You sell in BIUSDB" }) as HTMLInputElement;
    ...
    expect(screen.getByRole("button", { name: "Sell 5.05 BIUSDB" })).toBeDisabled();

    fireEvent.change(receiveInput,{target:{value:"605"}});
    expect(sellInput.value).toBe("6.05");
    expect(screen.getByRole("button", { name: "Sell 6.05 BIUSDB" })).toBeEnabled();
  });
});
