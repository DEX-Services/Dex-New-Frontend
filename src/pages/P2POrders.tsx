...
function settlementLabel(order: P2POrder, bought: boolean) {
  if (order.status === "completed") {
    return bought
      ? `Received ${formatBIUSDBAmount(order.buyerCreditRaw)} BIUSDB`
      : `Released ${formatBIUSDBAmount(order.buyerCreditRaw)} BIUSDB`;
  }
  if (order.status === "cancelled") {
    return bought
      ? "No BIUSDB received"
      : `Refunded ${formatBIUSDBAmount(order.sellerDebitRaw)} BIUSDB`;
  }
  return bought
    ? `Pending ${formatBIUSDBAmount(order.buyerCreditRaw)} BIUSDB`
    : `Escrowed ${formatBIUSDBAmount(order.sellerDebitRaw)} BIUSDB`;
}
      ...
      <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-sm">
        ...
        <tbody>{loading ? <tr><td colSpan={9} className="p-8 text-center">Loading…</td></tr> : orders.length === 0 ? <tr><td colSpan={9} className="p-8 text-center text-muted-foreground">No P2P orders found.</td></tr> : orders.map(order => {
          ...
          return <tr className="border-b last:border-0" key={order.id}>
            ...
            <td className="p-4">{formatBIUSDBAmount(order.amountRaw)} BIUSDB</td>
            <td className="p-4">{order.paymentMethod}</td>
            <td className="p-4">{formatINR(order.grossAmount)}</td>
            <td className="p-4">{formatBIUSDBAmount(bought ? order.buyerFeeRaw : order.sellerFeeRaw)} BIUSDB</td>
